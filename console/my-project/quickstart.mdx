...
[Projects](/identity-and-access-management/iam/concepts/#project) are Scaleway’s resource management feature. As the Owner of your Scaleway [Organization](/identity-and-access-management/iam/concepts/#organization), you can create multiple Projects. Grouping different resources (eg. Instances, Object Storage buckets, Managed Databases etc.) into different Projects allows you to define different access for each of them. Those access are defined through [permission sets](/identity-and-access-management/iam/concepts/#permission-set). 
...
To connect to Instances and other resources you create, you need to generate an SSH key for your Project. The advantage of creating and uploading an SSH key to your Project before you start creating resources, is that you are then prompted to associate the uploaded SSH key with the resource during the resource creation. This makes it easier for you to manage SSH access. 
